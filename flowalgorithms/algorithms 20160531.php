...
class FlowAlgorithmsLibrary extends FlowLibrary {
	...
	******************************************************************/  
	...
	//fn_TrackTypeFlow("FirstAssessment,NewOrKnown",1003,1002) - FieldID: 2251
	function fn_TrackTypeFlow($arrInputs = array()) {
		
		$NewOrKnown = $arrInputs[0];
		
		$HistoryAndSymptomsFlow = $arrInputs[1];
		$DiagnosisFTFlow = $arrInputs[2];
	...
	//fn_SpiroOrPEFFlow("AssessmentDetails,AssessmentType","LungFunction,LungFunctionOutcome","Spirometry,MaximumEffort","LungFunction,LungFunctionPerformed",1008,5004,20000) - FieldID: 2246	
	public function fn_SpiroOrPEFFlow($arrInputs = array()) {
		...
		$AssessControlFlow = $arrInputs[4];
		$SummarySoFarFlow = $arrInputs[5];
		$ExitFlow = $arrInputs[6];

		if ($LungFunctionOutcome != "Spirometry performed - Restrictive"
		...
		} elseif ($LungFunctionOutcome == "Spirometry performed - Restrictive" || $MaximumEffort == "N" || substr($LungFunctionOutcome,0,5) == "Error") {
			return $ExitFlow;
		} else {
			$CI = & get_instance();
	...
	function fn_TherapyOrDeviceChanged($arrInputs = array()) {
		...
		*/
		...
		$TherapyOrDeviceChangedFlow = $arrInputs[38];
		$StartingNewTherapyFlow = $arrInputs[38];
		$TherapyOrDeviceNotChangedFlow = $arrInputs[39];
		$JustStartingTherapy = $arrInputs[39];
		$JustFinishingTherapy = $arrInputs[39];
		$NeverOnTherapy = $arrInputs[39];

		//Changes to existing therapy (CurrentMedication different to MedicationReview for any non-empty drug type)
		if (
			   ($CurrentMedication_CurrentDrugCombNewDrug != $MedicationReview_ReviewDrugCombNewDrug && !empty($CurrentMedication_CurrentDrugCombNewDrug) && !empty($MedicationReview_ReviewDrugCombNewDrug))
			|| ($CurrentMedication_CurrentDrugCromolynsNewDrug != $MedicationReview_ReviewDrugCromolynsNewDrug && !empty($CurrentMedication_CurrentDrugCromolynsNewDrug) && !empty($MedicationReview_ReviewDrugCromolynsNewDrug))
			|| ($CurrentMedication_CurrentDrugICSNewDrug != $MedicationReview_ReviewDrugICSNewDrug && !empty($CurrentMedication_CurrentDrugICSNewDrug) && !empty($MedicationReview_ReviewDrugICSNewDrug))
			|| ($CurrentMedication_CurrentDrugILAANewDrug != $MedicationReview_ReviewDrugILAANewDrug && !empty($CurrentMedication_CurrentDrugILAANewDrug) && !empty($MedicationReview_ReviewDrugILAANewDrug))
			|| ($CurrentMedication_CurrentDrugILABANewDrug != $MedicationReview_ReviewDrugILABANewDrug && !empty($CurrentMedication_CurrentDrugILABANewDrug) && !empty($MedicationReview_ReviewDrugILABANewDrug))
			|| ($CurrentMedication_CurrentDrugISAANewDrug != $MedicationReview_ReviewDrugISAANewDrug && !empty($CurrentMedication_CurrentDrugISAANewDrug) && !empty($MedicationReview_ReviewDrugISAANewDrug))
			|| ($CurrentMedication_CurrentDrugISABANewDrug != $MedicationReview_ReviewDrugISABANewDrug && !empty($CurrentMedication_CurrentDrugISABANewDrug) && !empty($MedicationReview_ReviewDrugISABANewDrug))
			|| ($CurrentMedication_CurrentDrugLTRANewDrug != $MedicationReview_ReviewDrugLTRANewDrug && !empty($CurrentMedication_CurrentDrugLTRANewDrug) && !empty($MedicationReview_ReviewDrugLTRANewDrug))
			|| ($CurrentMedication_CurrentDrugMucolyticNewDrug != $MedicationReview_ReviewDrugMucolyticNewDrug && !empty($CurrentMedication_CurrentDrugMucolyticNewDrug) && !empty($MedicationReview_ReviewDrugMucolyticNewDrug))
			|| ($CurrentMedication_CurrentDrugNebSAANewDrug != $MedicationReview_ReviewDrugNebSAANewDrug && !empty($CurrentMedication_CurrentDrugNebSAANewDrug) && !empty($MedicationReview_ReviewDrugNebSAANewDrug))
			|| ($CurrentMedication_CurrentDrugNebSABANewDrug != $MedicationReview_ReviewDrugNebSABANewDrug && !empty($CurrentMedication_CurrentDrugNebSABANewDrug) && !empty($MedicationReview_ReviewDrugNebSABANewDrug))
			|| ($CurrentMedication_CurrentDrugOBANewDrug != $MedicationReview_ReviewDrugOBANewDrug && !empty($CurrentMedication_CurrentDrugOBANewDrug) && !empty($MedicationReview_ReviewDrugOBANewDrug))
			|| ($CurrentMedication_CurrentDrugOralSteroidsNewDrug != $MedicationReview_ReviewDrugOralSteroidsNewDrug && !empty($CurrentMedication_CurrentDrugOralSteroidsNewDrug) && !empty($MedicationReview_ReviewDrugOralSteroidsNewDrug))
			|| ($CurrentMedication_CurrentDrugPDE4InhibitorNewDrug != $MedicationReview_ReviewDrugPDE4InhibitorNewDrug && !empty($CurrentMedication_CurrentDrugPDE4InhibitorNewDrug) && !empty($MedicationReview_ReviewDrugPDE4InhibitorNewDrug))
			|| ($CurrentMedication_CurrentDrugTheophyllineNewDrug != $MedicationReview_ReviewDrugTheophyllineNewDrug && !empty($CurrentMedication_CurrentDrugTheophyllineNewDrug) && !empty($MedicationReview_ReviewDrugTheophyllineNewDrug))
			|| ($CurrentMedication_CurrentDrugLongActingInjectableSteroidsNewDrug != $MedicationReview_ReviewDrugLongActingInjectableSteroidsNewDrug && !empty($CurrentMedication_CurrentDrugLongActingInjectableSteroidsNewDrug) && !empty($MedicationReview_ReviewDrugLongActingInjectableSteroidsNewDrug))
			|| ($CurrentMedication_CurrentDrugNebulisedSABASAMANewDrug != $MedicationReview_ReviewDrugNebulisedSABASAMANewDrug && !empty($CurrentMedication_CurrentDrugNebulisedSABASAMANewDrug) && !empty($MedicationReview_ReviewDrugNebulisedSABASAMANewDrug))
			|| ($CurrentMedication_CurrentDrugLABALAMANewDrug != $MedicationReview_ReviewDrugLABALAMANewDrug && !empty($CurrentMedication_CurrentDrugLABALAMANewDrug) && !empty($MedicationReview_ReviewDrugLABALAMANewDrug))
			|| ($CurrentMedication_CurrentDrugInjectableBetaAgonistNewDrug != $MedicationReview_ReviewDrugInjectableBetaAgonistNewDrug && !empty($CurrentMedication_CurrentDrugInjectableBetaAgonistNewDrug) && !empty($MedicationReview_ReviewDrugInjectableBetaAgonistNewDrug))
		) {
			echo "<script>console.log( 'TherapyOrDeviceChangedFlow' );</script>";
			return $TherapyOrDeviceChangedFlow;
		//Starting new therapy (MedicationReview not empty, CurrentMedication empty for any drug type)
		} elseif (
			   (empty($CurrentMedication_CurrentDrugCombNewDrug) && !empty($MedicationReview_ReviewDrugCombNewDrug))
			|| (empty($CurrentMedication_CurrentDrugCromolynsNewDrug) && !empty($MedicationReview_ReviewDrugCromolynsNewDrug))
			|| (empty($CurrentMedication_CurrentDrugICSNewDrug) && !empty($MedicationReview_ReviewDrugICSNewDrug))
			|| (empty($CurrentMedication_CurrentDrugILAANewDrug) && !empty($MedicationReview_ReviewDrugILAANewDrug))
			|| (empty($CurrentMedication_CurrentDrugILABANewDrug) && !empty($MedicationReview_ReviewDrugILABANewDrug))
			|| (empty($CurrentMedication_CurrentDrugISAANewDrug) && !empty($MedicationReview_ReviewDrugISAANewDrug))
			|| (empty($CurrentMedication_CurrentDrugISABANewDrug) && !empty($MedicationReview_ReviewDrugISABANewDrug))
			|| (empty($CurrentMedication_CurrentDrugLTRANewDrug) && !empty($MedicationReview_ReviewDrugLTRANewDrug))
			|| (empty($CurrentMedication_CurrentDrugMucolyticNewDrug) && !empty($MedicationReview_ReviewDrugMucolyticNewDrug))
			|| (empty($CurrentMedication_CurrentDrugNebSAANewDrug) && !empty($MedicationReview_ReviewDrugNebSAANewDrug))
			|| (empty($CurrentMedication_CurrentDrugNebSABANewDrug) && !empty($MedicationReview_ReviewDrugNebSABANewDrug))
			|| (empty($CurrentMedication_CurrentDrugOBANewDrug) && !empty($MedicationReview_ReviewDrugOBANewDrug))
			|| (empty($CurrentMedication_CurrentDrugOralSteroidsNewDrug) && !empty($MedicationReview_ReviewDrugOralSteroidsNewDrug))
			|| (empty($CurrentMedication_CurrentDrugPDE4InhibitorNewDrug) && !empty($MedicationReview_ReviewDrugPDE4InhibitorNewDrug))
			|| (empty($CurrentMedication_CurrentDrugTheophyllineNewDrug) && !empty($MedicationReview_ReviewDrugTheophyllineNewDrug))
			|| (empty($CurrentMedication_CurrentDrugLongActingInjectableSteroidsNewDrug) && !empty($MedicationReview_ReviewDrugLongActingInjectableSteroidsNewDrug))
			|| (empty($CurrentMedication_CurrentDrugNebulisedSABASAMANewDrug) && !empty($MedicationReview_ReviewDrugNebulisedSABASAMANewDrug))
			|| (empty($CurrentMedication_CurrentDrugLABALAMANewDrug) && !empty($MedicationReview_ReviewDrugLABALAMANewDrug))
			|| (empty($CurrentMedication_CurrentDrugInjectableBetaAgonistNewDrug) && !empty($MedicationReview_ReviewDrugInjectableBetaAgonistNewDrug))
		) {
			echo "<script>console.log( 'StartingNewTherapyFlow' );</script>";
			return $StartingNewTherapyFlow;
		//No changes to existing therapy (CurrentMedication same as MedicationReview for all non-empty drug types)
		} elseif (
			   ($CurrentMedication_CurrentDrugCombNewDrug == $MedicationReview_ReviewDrugCombNewDrug && !empty($CurrentMedication_CurrentDrugCombNewDrug) && !empty($MedicationReview_ReviewDrugCombNewDrug))
			|| ($CurrentMedication_CurrentDrugCromolynsNewDrug == $MedicationReview_ReviewDrugCromolynsNewDrug && !empty($CurrentMedication_CurrentDrugCromolynsNewDrug) && !empty($MedicationReview_ReviewDrugCromolynsNewDrug))
			|| ($CurrentMedication_CurrentDrugICSNewDrug == $MedicationReview_ReviewDrugICSNewDrug && !empty($CurrentMedication_CurrentDrugICSNewDrug) && !empty($MedicationReview_ReviewDrugICSNewDrug))
			...
			echo "<script>console.log( 'TherapyOrDeviceNotChangedFlow' );</script>";
			return $TherapyOrDeviceNotChangedFlow;
		//Never on therapy (All CurrentMedication and MedicationReview strings empty or NULL for all drug types)
		} elseif (
		...
		//Just starting therapy (All CurrentMedication strings empty or NULL for all drug types)
		} elseif (
			   empty($CurrentMedication_CurrentDrugCombNewDrug)
			&& empty($CurrentMedication_CurrentDrugCromolynsNewDrug)
			&& empty($CurrentMedication_CurrentDrugICSNewDrug)
		...
		//Just finishing therapy (All MedicationReview strings empty or NULL for all drug types)
		} elseif (
			   empty($MedicationReview_ReviewDrugCombNewDrug)
			&& empty($MedicationReview_ReviewDrugCromolynsNewDrug)
			&& empty($MedicationReview_ReviewDrugICSNewDrug)
	...
	******************************************************************/    
	...
	"FirstAssessment,HospitalReport",
	...
	"NoMedication,WhyStoppedOtherDetails") - FieldID: 2307
	*/
	...
	function fn_TherapyMessage($arrInputs = array()) {
		...
		*/
		...
		if($CurrentMedicationLevel == 0 && $msgCode != "NS"){
