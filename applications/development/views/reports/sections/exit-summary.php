<p><?php 
	...
	$PEF_AvailableNextTime = $objReview->PEF_AvailableNextTime;
	...
	//PEF Not Available
	} elseif ($exitScreen == "PEF" || $exitScreen == "AH") {
		
		echo "<p>" . $genderSubjectivePronounUpper . " presents for asthma assessment but was unable to perform any type of lung function measure today. ";
		
		if($PEF_AvailableNextTime == "Y") {
			echo "It's likely this is a temporary feature so " . $genderSubjectivePronounLower . " has been re-booked for a few weeks time. In the meantime " . $genderPossessivePronounLower . " care has been described in the previously usual way. ";
		} elseif ($PEF_AvailableNextTime == "N") {
			echo "The computer guided package is not suitable in this situation, which is unusual and can be associated with diagnostic issues. ";
			if ($SecondaryCareAsthmaReferral == "Y") {
				echo $genderSubjectivePronounUpper . " has been referred for specialist assessment to resolve the reasons behind this.</p>";
			} elseif ($SecondaryCareAsthmaReferral == "N") {
				echo $genderSubjectivePronounUpper . " has <b>not</b> been referred for specialist assessment to resolve the reasons behind this, with the reason: <i>". $ReasonForNoReferralTextOnly . "</i>.</p>";
			}				
		}
